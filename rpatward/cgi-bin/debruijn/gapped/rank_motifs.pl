...
my $writable_dir_path = "/var/www/cgi-bin/rpatward/debruijn/gapped/writable";
...
#    print "$motifs[$motif][0][0]<br>\n";
    ...
    my $con_name = "Consensus_".$i;
    $motif_hash{$con_name} = $motifs[$motif][0][0];
    $i++;
}
...
system("/usr/local/bin/fasta34 -m 10 -A -p -q -E 200 -H consensus_seqs.fasta $protein_file > $writable_dir_path/temp_fasta.out");

system("perl new_parse_fasta34.pl $writable_dir_path/temp_fasta.out ");

my $wc_op = `wc -l alignment.out`;

$wc_op =~ /(\d+)\s+/;

my $wc_count = $1;

if($wc_count < 2 ){  ## no motifs

    print "Sorry ... I could not find any valid motifs in your input !!!<br>\n";
}

else{

    open(MO, "valid_motifs.txt");

    foreach my $motif (<MO>){
	chomp $motif;
	if($motif ne ""){
	    print "Motif: $motif_hash{$motif}<br>\n";
	}
    }
    ...
    system("cp final_output.out /var/www/html/rpatward/debruijn/gapped/tmp/final_output_gapped.out");

    print "<br><br><a href='http://biokdd.informatics.indiana.edu/rpatward/debruijn/gapped/tmp/final_output_gapped.out'>Output</a><br>";
