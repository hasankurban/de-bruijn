...
#### Looking backward
    ...
    open(TCON, ">temp_consensus.fasta");
    print TCON ">Consensus_$motif_count \n$motif\n";
    close TCON;

## Process this consensus to find local alignment

    system("fasta34 -m 10 temp_consensus.fasta $protein_file > temp_fasta.out");
    system("perl parse_fasta34.pl temp_fasta.out");

    open(TIP, "temp_alignment.txt");
...
system("cp final_output.out /var/www/html/rpatward/debruijn/tmp/");

#system("rm alignment.out");

print "<a href='http://biokdd.informatics.indiana.edu/rpatward/debruijn/tmp/final_output.out'>Output</a><br>";
