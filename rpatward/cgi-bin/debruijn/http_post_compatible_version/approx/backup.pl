...
## 6 May 2006
## created from sim_batch_testing_2.pl
## taking care of reducing weights of visited nodes

# July 6, 2006
# Created from sim_batch_testing_3.pl
# Modification to prevent cycles within a motif using edge_dup

## 14 July 2006
## Created from debruijn/approx/backup.pl
## Modifications to create xml output

## 7 August 2006
## Added protein file name as a argument to consensus_to_multiple.pl 
## This is due to modification of consensus_to_multiple.pl to report position of motif in sequence
...
## Process this consensus to find local alignment
	...
	system("perl backup_new_parse_fasta34.pl $dir_path/temp_fasta.out $dir_path");
...
#    print "Operation timed out";
    print "<error>Operation timed out</error>";
...
if($atleast_one_motif == 1){
    ...
    system("perl consensus_to_multiple_121706.pl $dir_path/alignment.out $dir_path/consensus_seqs.fasta $subs_matrix $dir_path $protein_file");
...
#    print "<br><a href='http://biokdd.informatics.indiana.edu/rpatward/debruijn/approx/tmp/$rand/final_output.out'>Output</a><br>";

}
else{

    print "<error>No motif found</error>";
