...
## Revision: take care of cases where all alignments start farther than the motif start
...
my $writable_dir_path = $ARGV[3];

my $protein_file = $ARGV[4];  ## input protein file
...
my %protein_seqs = read_fasta_file($protein_file);
...
#open(OP, ">$writable_dir_path/final_output.out");
...
while (my $line = <PAL>){
	...
	if ($line =~ /\*Consensus_(\d+)\*/) {  ## get corr consensus seq
	    ...
	    my $end = length($consensus) - ($start + length($con_part));
	    ...
	    # Find the start coordinate of motif in input protein sequence
	    my $temp_motif_seq = $temp2[1];
	    $temp_motif_seq =~ s/-//g;

	    $aln_start{$temp2[0]} = index($protein_seqs{$temp2[0]}, $temp_motif_seq) - $start;  #includes preceeding gaps
...
#	    print OP "consensus           \t$consensus_new\n\n";
	    ...
	    my $consensus_new_modified = substr($consensus_new,$min_start);
	    $consensus_new_modified = substr($consensus_new_modified, 0, length($consensus_new_modified)-$max_end);

	    print "<motif consensus='$consensus_new_modified'>";
...
#	    print "<motif>";    
		...
		$aln_seq{$seq_id} =~ s/\./-/g;
		my $aln_seq_modified = substr($aln_seq{$seq_id},$min_start);
		$aln_seq_modified = substr($aln_seq_modified, 0, length($aln_seq_modified)-$max_end);

		my $aln_start_adjusted = $aln_start{$seq_id} + $min_start;
=head		
...
##		print "<protein><id>$seq_id</id><region>$aln_seq{$seq_id}</region><index>$aln_start{$seq_id}</index></protein>";
		print "<protein><id>$seq_id</id><region>$aln_seq_modified</region><index>$aln_start_adjusted</index></protein>";
...
sub read_fasta_file { 
    ...
    open(SEQ, "$file_name") or die("<error>cannot open fasta file $file_name</error>");
	...
	if (substr($line,0,1) eq ">"){  # header
	    ($header)=($line=~ />(\S+)/);  ## considering only the first word
	    $seqs{$header}="";
	}
	else{    #line is seq
